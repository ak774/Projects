...
app = Flask(__name__)
model = pickle.load(open('linear_ass.pickle', 'rb'))
model_scale = pickle.load(open('scale_lin.pickle','rb'))
model_descale = pickle.load(open('lin_scale_y.pickle','rb'))
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        col = ['CRIM', 'ZN', 'INDUS', 'CHAS', 'NOX', 'AGE', 'DIS', 'RAD', 'TAX',
               'PTRATIO', 'B']
        for i in col:
            csv_shape[i] = np.log(csv_shape[i], out=np.zeros_like(csv_shape[i]), where=(csv_shape[i]!= 0))
        scaled=model_scale.transform(csv_shape)
        pred=model.predict(scaled)
        delog= np.exp(pred, out=np.zeros_like(pred), where=(pred!= 0))
        descale=model_descale.inverse_transform(delog)
        return render_template('index.html', csv_shape=pd.DataFrame(descale).to_html())
